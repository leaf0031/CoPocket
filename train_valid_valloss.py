import sys
sys.path.append('/home/disk2/xxr/now/HGNN-GPT-last-new-2__2/')
import argparse
import yaml
import os
import random
...
from model.model import CoPocket
import warnings
...
from utils.data import collate_mols
import pickle
...
def configure_optimizers(self, train_config): 
    ...
    if unclassified:
        print(f"Warning: The following parameters are unclassified and will not undergo weight decay by default: {unclassified}")
...
if __name__ == "__main__":
    ...
    for epoch in range(1, 1 + num_epoch):
        ...
        for gnn_data, hgnn_data in trainloader:
            optimizer.zero_grad()
            gnn_data = gnn_data.to(device, non_blocking=True)
            ...
            base_path = '/home/disk2/xxr/now/data/crossdocked_pocket10_3DAtomF/'
            mol_dicts = []

            files = [os.path.join(base_path, name.replace('_pocket10.pdb', '.pkl')) for name in gnn_data.pocket_name]
            for file in files:
                with open(file, 'rb') as f:
                    data = pickle.load(f)
                mol_dicts.append(data)

            batch = collate_mols(mol_dicts)

            if 'protein_filename' in batch:
                del batch['protein_filename']
            
            for key in batch:
                batch[key] = batch[key].to(device)

            # forward  
            preds, _, cl_loss= model(
                gnn_data, 
                hgnn_data, 
                smiles, 
                lengths,
            ...
            del gnn_data, hgnn_data, smiles, input, targets, lengths, batch, preds, loss, cl_loss
        ...
        for gnn_data, hgnn_data in valloader:
            ...
            batch = collate_mols(mol_dicts)
            # Delete the protein_filename key from the batch.
            if 'protein_filename' in batch:
                del batch['protein_filename']
        ...
        print('epoch {}, train loss: {}, val loss: {}, f_loss: {}.'.format(
            epoch, train_losses[-1], val_losses[-1], cl_losses[-1]))
        ...
        if valid_all[-1] > 0.95:
            print(f" Early stopping at epoch {epoch} because valid = {valid_all[-1]:.4f} > 0.95")
            break
