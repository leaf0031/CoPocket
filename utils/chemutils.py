...
def kabsch_torch(A, B, C):
    """
    The Kabsch algorithm is used to align point set A to point set B and apply the same rotation and translation to point set C.

    Parameters:
    - A: Original point set (Tensor, shape [B, N, 3]), where B is the batch size, N is the number of points, and 3 is the number of dimensions of the coordinates.
        - B: Target point set (Tensor, shape [B, N, 3]).
    - C: Point set to be aligned (Tensor, shape [B, M, 3]), with the same rotation and translation applied as from A to B.

    Returns:
    - C_aligned: Aligned point set C (Tensor, shape [B, M, 3]).
    - R: Rotation matrix (Tensor, shape [B, 3, 3]).
- t: Translation vector (Tensor, shape [B, 3]).
    """
    A=A.double()
    B=B.double()
    C=C.double()
    ...
    if torch.isnan(B).any():
        print("Warning: B contains NaN values. Replacing NaNs with zeros.")
        B = torch.nan_to_num(B, nan=0.0) 
        # B = torch.where(torch.isnan(B), B.mean(dim=1, keepdim=True), B)
    ...
    epsilon = 1e-6  
    H += epsilon * torch.eye(3, device=H.device)  
