...
class FeaturizeProteinAtom(object):
    ...
    def __call__(self, data: ProteinLigandData):
        element = data['protein_element'].view(-1, 1) == self.atomic_numbers.view(1, -1)  # (N_atoms, N_elements)（原子数，原子类型）
...
class LigandBFSMask(object):
    ...
    def get_bfs_perm_motif(moltree, vocab):
        ...
        while len(bfs_queue) > 0:
            ...
            bfs_queue = bfs_queue + next_candid
    ...
    def __call__(self, data):
        ...
        if isinstance(data['protein_contact'], torch.Tensor):
            data['protein_contact'] = data['protein_contact'].clone().detach()
        else:
            data['protein_contact'] = torch.tensor(data['protein_contact']).clone().detach()
